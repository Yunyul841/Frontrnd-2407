...
function area() {let radius = 5; return PI * radius ** 2;}

// 복합 대입 연산자
let a = 0, str = '';
a += 1; a += 2, a += 3;                   // a <- a + 1;
console.log(a);
str += 'A quick brown fox ';                  // str  <- 'a quick brown fox'
...
// 4. undefiend 자료형
let c;
console.log(typeof(abc), typeof(c));          // 선언하지 않는 변수, 값을 초기화하지 않은 변수
...
console.log(Boolean(0), Boolean(0.0), Boolean(''), Boolean(undefined), Boolean([]), Boolean({}));       // false, 이 경우를 제외하곤 true

// 자동 자료 변환
console.log("ABC" + 100 + true);       // 표현법위가 작은 것이 큰 것으로 자동적으로 변환됨
console.log(100 + true);               // true > 1, false > 0
